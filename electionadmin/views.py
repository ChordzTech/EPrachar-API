...
class AdminAPI(ModelViewSet):
     ...
     def perform_create(self, serializer):
          ...
          if image_file:
               ...
               image_name = f"admin_{user_id}.{extension}"

               upload_dir = os.path.join(settings.MEDIA_ROOT, 'admin_images')
               os.makedirs(upload_dir, exist_ok=True)
               image_path = os.path.join(upload_dir, image_name)

               # Save file manually (works for SVG too)
               with open(image_path, 'wb+') as dest:
                    for chunk in image_file.chunks():
                         dest.write(chunk)

               instance.a_image.name = f"admin_images/{image_name}"
               instance.save()
     ...
     def perform_update(self, serializer):
          ...
          if image_file:
               ...
               if instance.a_image and os.path.isfile(instance.a_image.path):
                    os.remove(instance.a_image.path)
